...
`;
    ...
    let EditBtn = listItem.getElementsByClassName("edit_btn")[0];
    let DeleteBtn = listItem.getElementsByClassName('delete_btn')[0];
...
function displayProducts(product) {
  ...
  // Display specific fields from the product
  listItem.innerHTML = `
<img src="${product.imageAddr}" alt="${product.title}" style="max-width: 100px; max-height: 100px;">
<div>
  <h5>Title: ${product.title}</h5>
  <br>
  <p>Description: <input class="desInput" style="width: 720px; height: 40px;" type="text" value="${product.description}"></p>
  <label>Price ($): <input style="margin-left:25px;" type="number" class="priceInput" name="positiveNumber" min="1" pattern="[0-9]*" value="${product.price}" required></label>
  <br><br>
  <label>Quantity(U): <input type="number" class="qntInput" name="positiveNumber" min="1" pattern="[0-9]*" value="${product.quantity}" required></label>
  <br><br>
  <button class='edit_btn'>Edit</button>
  <button class='delete_btn'>Delete</button>
</div>
